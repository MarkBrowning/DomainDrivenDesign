...
{
    public class Domain : Composite
    {
        public Domain() : base() { }     
        public Domain(String name) : base() { this.Type = "Domain"; this.Name = name; }
        public Domain(String name, String value) : base() { this.Type = "Domain"; this.Name = name; this.Value = value; }

        #region SubDomains
        public List<Domain> SubDomains { get { return this.getType<Domain>(); } }

        public Domain getSubDomain(String name)
        { return this.getType<Domain>().FirstOrDefault(e => e.Name == name); }

        public void addSubDomain(String name)
        {
            if (this.getType<Domain>().FirstOrDefault(e => e.Name == name) == null)
                this.addChild(new Domain(name));
        }

        public void addSubDomain(Domain subDomain)
        {
            if (this.getType<Domain>().FirstOrDefault(e => e.Name == subDomain.Name) == null)
                this.addChild(subDomain);
        }
        public void removeSubDomain(String name)
        {
            Domain subDomain = this.getType<Domain>().FirstOrDefault(e => e.Name == name);

            if (subDomain != null)
                this.removeChild(subDomain);
        }
